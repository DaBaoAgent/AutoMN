...
export const providerModule = { name: "@volcengine/provider-ark-seedream", version: "1" } as const;
...
const implementedPorts: readonly string[] =
  ["prompt", "aspectRatio", "quality", "outputFormat", "nsfwCheck", "images"];

type Quality = "basic" | "high" | "ultra";

const RATIOS: Readonly<Record<string, readonly [number, number]>> = {
  "1:1": [1, 1], "4:3": [4, 3], "3:4": [3, 4], "16:9": [16, 9],
  "9:16": [9, 16], "2:3": [2, 3], "3:2": [3, 2], "21:9": [21, 9],
};

/** 长边目标；Ark 只接受 WIDTHxHEIGHT（或个别预设），所以由画幅 + 档位算出具体像素。 */
const longSideByQuality: Readonly<Record<Quality, number>> = { basic: 1536, high: 2048, ultra: 2560 };
/** 实测：1440x2560(3.7M px) 通过，2304x4096(9.4M px) 被拒；留出余量。 */
const maxAreaPx = 4_300_000;
const minSidePx = 640;
const stepPx = 16;

export function arkImageSize(aspectRatio: string, quality: Quality): string {
  const ratio = RATIOS[aspectRatio];
  if (ratio === undefined) throw new Error(`Ark Seedream 不支持画幅 ${aspectRatio}`);
  const long = longSideByQuality[quality];
  const [a, b] = ratio;
  const snap = (value: number) => Math.max(stepPx, Math.round(value / stepPx) * stepPx);
  ...
  if (width < minSidePx) { height = snap((height * minSidePx) / width); width = minSidePx; }
  if (height < minSidePx) { width = snap((width * minSidePx) / height); height = minSidePx; }
...
function address(value: string): string {
  ...
  if (url.protocol !== "https:") throw new Error("Ark baseUrl requires HTTPS");
...
export function createArkSeedreamProvider(options: {
  ...
  function support(request: EndpointRequest) {
    ...
    if (typeof quality === "string" && !(quality in longSideByQuality)) {
      return { status: "unsupported" as const, reason: `Ark Seedream 不接受档位 ${quality}` };
    }
    const aspectRatio = ports.aspectRatio?.[0];
    if (typeof aspectRatio === "string" && !(aspectRatio in RATIOS)) {
      return { status: "unsupported" as const, reason: `Ark Seedream 不接受画幅 ${aspectRatio}` };
    }
  ...
  async function inlineReference(context: Parameters<NonNullable<AsyncEndpoint["start"]>>[0],
    media: GenerationMediaValue, label: string): Promise<string> {
    const bytes = await context.resources.get(media.artifact.resource);
    if (bytes === undefined) throw new Error(`参考图 ${label} 不可用`);
    if (bytes.byteLength > options.maxReferenceBytes) {
      throw new Error(
        `参考图 ${label} 为 ${(bytes.byteLength / 1024 / 1024).toFixed(1)}MB，`
  ...
  const endpoint: AsyncEndpoint = {
    async start(context) {
      ...
      const quality = (ports.quality?.[0] ?? "high") as Quality;
      ...
      if (envelope === undefined) throw new Error("Ark /images/generations 返回了非 JSON 响应");
      ...
      if (!media.ok) throw new Error(`Ark 图片下载返回 HTTP ${media.status}`);
  ...
  return defineEndpointPackage({
    ...
    async readPricing(context) {
      const ports = (context.request.constraints as unknown as GenerationRequest).ports;
      const quality = (ports.quality?.[0] ?? "high") as Quality;
