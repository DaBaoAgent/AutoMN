...
const KINDS = ["multi-image", "text-to-video", "first-last", "image-audio"];
function fail(message) {
    throw new Error(`AutoDL H3 配置错误：${message}`);
}
function stringList(value, subject) {
    if (!Array.isArray(value))
        fail(`${subject} 必须是字符串数组`);
    return value.map((item) => {
        if (typeof item !== "string" || item.length === 0)
            fail(`${subject} 里必须是字符串`);
        return item;
    });
}
function parseSpec(value, index) {
