...
export const providerModule = { name: "@autodl/provider-autodl-h3", version: "1" };
/**
...
const implementedPorts = ["prompt", "duration", "resolution", "aspectRatio", "referenceImage", "referenceAudio", "firstFrame", "lastFrame"];
...
function address(value) {
    ...
    if (url.protocol !== "https:")
        throw new Error("AutoDL baseUrl requires HTTPS");
...
function kindOf(ports) {
    const has = (port) => (ports[port]?.length ?? 0) > 0;
    if (has("referenceVideo"))
        return undefined;
    if (has("firstFrame") || has("lastFrame"))
        return "first-last";
    if (has("referenceAudio"))
        return "image-audio";
    if (has("referenceImage"))
        return "multi-image";
...
export function createAutodlH3Provider(options) {
    ...
    if (options.workflows.length === 0)
        throw new Error("AutoDL H3 requires at least one workflow");
    ...
    function analyze(request) {
        ...
        if (unknownPort !== undefined) {
            return { ok: false, reason: `AutoDL 的 H3 工作流没有 ${unknownPort} 端口的线上映射` };
        }
        const kind = kindOf(ports);
        if (kind === undefined) {
            return { ok: false, reason: "AutoDL 已登记的 H3 工作流都不吃参考视频（referenceVideo）" };
        }
        ...
        const candidates = ranked
            ...
            .filter((spec) => spec.resolutions.length === 0 || spec.resolutions.some((entry) => entry.tier === tier && (aspectRatio === undefined || entry.aspectRatio === aspectRatio)))
        ...
        const resolution = spec.resolutions.find((entry) => entry.tier === tier && (aspectRatio === undefined || entry.aspectRatio === aspectRatio));
        return { ok: true, spec, resolution, ports, kind };
    }
    function support(request) {
        const analyzed = analyze(request);
        return analyzed.ok
            ? { status: "supported" }
            : { status: "unsupported", reason: analyzed.reason };
    }
    /** One AutoDL call. The service answers HTTP 200 with a `code` envelope on the body. */
    async function call(path, secret, init = {}) {
    ...
    async function inlineMedia(context, media, label) {
        ...
        if (bytes.byteLength > options.maxReferenceBytes) {
            throw new Error(`${label} 为 ${(bytes.byteLength / 1024 / 1024).toFixed(1)}MB，`
    ...
    const endpoint = {
        async start(context) {
            ...
            if (typeof duration === "number" && spec.durations.length > 0)
                payload.duration = duration;
            if (analyzed.resolution !== undefined)
                payload.resolution = analyzed.resolution.payload;
            const references = (ports.referenceImage ?? []);
            ...
            if (spec.kind === "first-last") {
                const first = ports.firstFrame?.[0];
                const last = ports.lastFrame?.[0];
                if (first !== undefined)
                    payload.first_frame = await inlineMedia(context, first, "首帧");
                if (last !== undefined)
                    payload.last_frame = await inlineMedia(context, last, "尾帧");
            }
            if (spec.kind === "image-audio") {
                const audios = (ports.referenceAudio ?? []);
                const audio = audios[0];
                if (audio === undefined)
                    throw new Error("对口型工作流需要一段参考音频");
                payload[spec.audioField ?? "ref_audio_0"] = await inlineMedia(context, audio, "参考音频");
                if (typeof duration === "number")
                    payload[spec.audioDurationField ?? "audio_duration"] = duration;
            }
        ...
        async poll(context) {
            ...
            const secret = text(context.credentials.apiKey?.secret, "AutoDL API Key");
            const { data, message } = await call(`/api/v1/comfyui/comfyui_workflow/result/${encodeURIComponent(id)}`, secret);
            ...
            return wakeAfter(handle, interval, Date.now(), {
        ...
        async collect(context) {
            ...
            if (!response.ok)
                throw new Error(`AutoDL 成片下载返回 HTTP ${response.status}`);
